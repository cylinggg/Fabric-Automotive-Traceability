...
Peer endorsement (which organisations' peers must sign), caller
authorisation (which client identity may invoke), and business
preconditions (ordinary, caller-supplied application data) are three
distinct mechanisms — see the chaincode's top-of-file comments and the
dissertation's Section 5 for why they should not be conflated. As of
